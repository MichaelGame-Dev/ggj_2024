...
const JokePlayer = preload("res://addons/sound_manager/joke.gd")
...
var joke: JokePlayer = JokePlayer.new(["Joke"], 8)
...
func set_joke_volume(volume_between_0_and_1) -> void:
	_show_shared_bus_warning()
	AudioServer.set_bus_volume_db(AudioServer.get_bus_index(joke.bus), linear_to_db(volume_between_0_and_1))
...
func play_joke(resource: AudioStream, crossfade_duration: float = 0.0, override_bus: String = "") -> AudioStreamPlayer:
	return joke.play(resource, override_bus)
