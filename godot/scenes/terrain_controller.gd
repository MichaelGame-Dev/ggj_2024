...
var terrain_belt: Array[MeshInstance2D] = []
@export var terrain_velocity: float = 100.0
...
func _init_blocks(number_of_blocks: int) -> void:
	for block_index in number_of_blocks:
		...
		if block_index == 0:
			block.position.y = block.mesh.size.y/2
			print("block position: ",block.position)
		else:
...
func _progress_terrain(delta: float) -> void:
	...
	#	block.position.y += terrain_velocity * delta

	if terrain_belt[0].position.y >= Globals.player.position.y + abs(terrain_belt[0].mesh.size.y/2):
		print("DELETEING BLOCK, terrain ", terrain_belt[0].position.y, " player + mesh size: ", Globals.player.position.y, " ", abs(terrain_belt[0].mesh.size.y/2))
...
func _append_to_far_edge(target_block: MeshInstance2D, appending_block: MeshInstance2D) -> void:
	appending_block.position.y = target_block.position.y - target_block.mesh.size.y/2 - appending_block.mesh.size.y/2
